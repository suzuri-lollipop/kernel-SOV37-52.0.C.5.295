...
extern void cam_vfe_bus_ver2_dbg_dump_wm_addrs(const char *tag);
	...
	enum cam_cdm_cb_status status, uint64_t cookie)
{
	...
	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		...
		cam_vfe_bus_ver2_dbg_dump_wm_addrs("cdm_bl_done");
	} else {
		CAM_WARN(CAM_ISP,
			"Called by CDM hdl=%x, udata=%pK, status=%d, cookie=%llu",
			 handle, userdata, status, cookie);
					...
					void *config_hw_args)
{
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		/* SOV37 debug: dump BL command buffer contents before submit */
		{
			static int dbg_bl_dump_cnt;
			int bi;

			if (dbg_bl_dump_cnt < 6) {
				dbg_bl_dump_cnt++;
				for (bi = 0; bi < cdm_cmd->cmd_arrary_count; bi++) {
					uint64_t bcpu = 0;
					size_t blen = 0;
					int32_t bh =
						cdm_cmd->cmd[bi].bl_addr.mem_handle;
					uint32_t boff = cdm_cmd->cmd[bi].offset;
					uint32_t bln = cdm_cmd->cmd[bi].len;
					uint32_t *bw;
					int wi, nwords;

					CAM_INFO(CAM_ISP,
						"BLDUMP req=%llu bl[%d] hdl=%d off=%u len=%u",
						cfg->request_id, bi, bh, boff, bln);

					if (cam_mem_get_cpu_buf(bh, &bcpu, &blen)) {
						CAM_INFO(CAM_ISP,
							"BLDUMP cannot map bl %d", bi);
						continue;
					}
					bw = (uint32_t *)((uint8_t *)bcpu + boff);
					nwords = bln / 4;
					if (nwords > 40)
						nwords = 40;
					for (wi = 0; wi < nwords; wi += 4) {
						CAM_INFO(CAM_ISP,
							"BLDUMP req=%llu bl[%d] +%03x: %08x %08x %08x %08x",
							cfg->request_id, bi, wi * 4,
							(wi + 0 < nwords) ? bw[wi + 0] : 0,
							(wi + 1 < nwords) ? bw[wi + 1] : 0,
							(wi + 2 < nwords) ? bw[wi + 2] : 0,
							(wi + 3 < nwords) ? bw[wi + 3] : 0);
					}
				}
			}
		}
...
int cam_ife_hw_mgr_init(struct cam_hw_mgr_intf *hw_mgr_intf, int *iommu_hdl)
{
	...
	if (!cam_cdm_get_iommu_handle("ife", &cdm_handles) &&
		...
		g_ife_hw_mgr.mgr_common.cmd_iommu_hdl_secure =
			cdm_handles.secure;
	} else if (!cam_smmu_get_handle("cpas-cdm0",
			&g_ife_hw_mgr.mgr_common.cmd_iommu_hdl)) {
		CAM_INFO(CAM_ISP,
			"CDM cmd iommu handle via cpas-cdm0 fallback (virtual cdm)");
		if (cam_smmu_ops(g_ife_hw_mgr.mgr_common.cmd_iommu_hdl,
			CAM_SMMU_ATTACH))
			CAM_WARN(CAM_ISP, "cpas-cdm0 attach failed (may be ok)");
